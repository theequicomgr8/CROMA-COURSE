    ...
    <div class="container">
        <div class="tab-list-flex">
           <div class="tabbox-css create-new-course-curricullumform">
            <h3>Edit Course Curriculum</h3>
               <div class="form-changepassword">
                  <form method="post" action="{{Route('curriculm.update')}}" enctype="multipart/form-data">
                      @csrf
                      
                    
                    <input type="hidden" name="coid" value="{{Request::segment(2)}}">
                    <div class="form-login mb-0" style="pointer-events: none;">
                        <div class="row">
                           <div class="col-lg-12">
                              <label class="form-label">Category</label>
                              <select name="category_id" class="my-from-control selectcol choosecategory" aria-label="Select Category">
                                 <option value="">Select Category</option>
                                 @foreach($categorydata as $value)
                                 <option value="{{$value['category_id']}}" {{@$templatedata->category_id==$value['category_id'] ? 'selected' : '' }} >{{$value['category_name']}}</option>
                                 @endforeach
                              </select>
                           </div>
                     ...
                     <div class="form-login mb-0" style="pointer-events: none;">
                        <div class="row">
                           <div class="col-lg-12">
                              ...
                              <select name="course_id" class="my-from-control selectcol choosecourse" aria-label="Select Course" >
                                 <option value="">Select Course</option>
                                 @if(!empty($getcourse))
                                    @foreach($getcourse as $value)
                                        <option value="{{$value['id']}}" {{$value['id']==Request::segment(2) ? 'selected' : '' }}>{{$value['course_name']}}</option>
                                    @endforeach
                                 @else
                                    @foreach($getcourse as $value)
                     ...
                     <div class="form-login mb-0">
                        <div class="row">
                           <div class="col-lg-12">
                              ...
                              <select name="header_id" class="my-from-control selectcol" aria-label="Select Header ">
                                 <option value="">Select Header</option>
                                 @foreach($header as $value)
                                 <option value="{{$value->id}}" {{@$templatedata->header_id==$value->id ? 'selected' : '' }}>{{$value->name}}</option>
                                 @endforeach
                              </select>
                           </div>
                     ...
                     <div class="form-login mb-0">
                        <div class="row">
                           <div class="col-lg-12">
                              ...
                              <select name="footer_id" class="my-from-control selectcol" aria-label="Select Footer">
                                 <option value="">Select Footer</option>
                                 @foreach($footer as $value)
                                 <option value="{{$value->id}}" {{@$templatedata->footer_id==$value->id ? 'selected' : '' }} >{{$value->name}}</option>
                                 @endforeach
                              </select>
                           </div>
                        ...
                        <div class="row">
                           <div class="col-lg-12">
                              ...
                              <div class="upload-excel text-center">
                                  <input type="file" name="excelfile" id="real-file" hidden="hidden" accept=".csv, application/vnd.openxmlformats-officedocument.spreadsheetml.sheet, application/vnd.ms-excel" />
           ...
           <div class="pdfviewerbox">
            ...
            <embed src= "https://course.cromacampus.com/public/loadpdf/{{Request::segment(2)}}.pdf#toolbar=0&navpanes=0&scrollbar=0" width= "100%" height= "500px" frameborder="0">
              <div class="text-center downloadpdf mt-3">
               <a href="https://course.cromacampus.com/public/loadpdf/{{Request::segment(2)}}.pdf" target="_blank" class="btn btn-third">
